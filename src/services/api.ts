...
async function callGemini(prompt: string): Promise<string> {
  if (!genAI) throw new Error("AI not configured. Set GEMINI_API_KEY in .env.local");
...
export const bulkCreateTickets = async (ticketsData: Partial<Ticket>[], requesterId: string): Promise<void> => {
  const dbData = ticketsData.map(t => ({
    ...
    requester_id: t.requester?.id || requesterId,
...
export const bulkCreateAssets = async (assetsData: Partial<Asset>[]): Promise<void> => {
  const dbData = assetsData.map(a => ({
    ...
    purchase_cost: a.purchaseCost, current_value: a.currentValue,
...
export const saveIntegration = async (userId: string, serviceName: string, apiKey: string): Promise<void> => {
  const { error } = await supabase.from('user_integrations').upsert({
    user_id: userId, service_name: serviceName, api_key: apiKey
...
export const deleteIntegration = async (userId: string, serviceName: string): Promise<void> => {
  const { error } = await supabase.from('user_integrations').delete()
    .eq('user_id', userId).eq('service_name', serviceName);
  if (error) throw error;
};
...
export const getVendors = async (): Promise<Vendor[]> => {
  ...
  return data.map(v => ({
    ...
    phone: v.phone, website: v.website, address: v.address, notes: v.notes,
    tags: v.tags, contractStartDate: v.contract_start_date,
    contractEndDate: v.contract_end_date, contractUrl: v.contract_url,
    serviceCount: v.service_count, avgResponseTime: v.avg_response_time
  }));
};
...
export const updateVendor = async (id: string, vendor: Partial<Vendor>): Promise<void> => {
  ...
  if (vendor.email) dbData.email = vendor.email;
  if (vendor.phone) dbData.phone = vendor.phone;
  if (vendor.website) dbData.website = vendor.website;
  if (vendor.address) dbData.address = vendor.address;
  if (vendor.notes) dbData.notes = vendor.notes;
...
export const getVendorInteractions = async (vendorId: string): Promise<VendorInteraction[]> => {
  ...
  if (error) return [];
  return data.map(i => ({
    id: i.id, vendorId: i.vendor_id, subject: i.subject, status: i.status,
    createdAt: i.created_at, resolvedAt: i.resolved_at, scheduledDate: i.scheduled_date
  }));
};

export const getVendorComments = async (interactionId: string): Promise<VendorComment[]> => {
  const { data, error } = await supabase.from('vendor_comments').select('*')
    .eq('interaction_id', interactionId).order('created_at', { ascending: true });
  if (error) return [];
  const users = await getUsers();
  const usersMap = new Map(users.map(u => [u.id, u]));
  return data.map(c => ({
    id: c.id, content: c.content, author: usersMap.get(c.author_id)!,
    isEmail: c.is_email, createdAt: c.created_at, attachments: c.attachments
  }));
};
...
export const createVendorInteraction = async (vendorId: string, subject: string, userId: string, scheduledDate?: string): Promise<VendorInteraction> => {
  const { data, error } = await supabase.from('vendor_interactions').insert({
    vendor_id: vendorId, subject, created_by: userId, status: 'Open',
    scheduled_date: scheduledDate || null
  }).select().single();
  if (error) throw error;
  return {
    id: data.id, vendorId: data.vendor_id, subject: data.subject,
    status: data.status, createdAt: data.created_at, scheduledDate: data.scheduled_date
  };
};
...
export const addVendorComment = async (interactionId: string, content: string, userId: string, isEmail: boolean, attachments: string[]): Promise<VendorComment> => {
  const { data, error } = await supabase.from('vendor_comments').insert({
    interaction_id: interactionId, content, author_id: userId,
    is_email: isEmail, attachments
  }).select().single();
  if (error) throw error;
  const users = await getUsers();
  const user = users.find(u => u.id === userId)!;
  return {
    id: data.id, content: data.content, author: user,
    isEmail: data.is_email, createdAt: data.created_at, attachments: data.attachments
  };
};
...
export const closeVendorInteraction = async (interactionId: string): Promise<void> => {
  const { error } = await supabase.from('vendor_interactions')
    .update({ status: 'Closed' }).eq('id', interactionId);
  if (error) throw error;
};
...
export const generateTicketReply = async (ticket: Ticket, history: string): Promise<string> => {
  const prompt = `You are a helpful IT support agent. Draft a professional reply to this ticket.
Ticket Subject: ${ticket.subject}
Description: ${ticket.description}
Conversation History:
${history}

Keep it concise, professional and helpful. Do not add subject line.`;
  try {
...
export const generatePostMortemSummary = async (incident: Incident): Promise<Partial<Incident>> => {
  const prompt = `Generate a post-mortem analysis for this IT incident. Respond ONLY with valid JSON, no markdown.
Title: ${incident.title}
Description: ${incident.description}
Timeline: ${JSON.stringify(incident.timeline)}

Return JSON with exactly these fields: { "rootCauseAnalysis": "...", "impactAnalysis": "...", "solution": "..." }`;
  try {
    ...
    console.error("Post-mortem parse error:", e);
    return { rootCauseAnalysis: "Failed to generate analysis. Please write manually." };
  }
};
...
export const getAIWritingAssistance = async (
  text: string,
  language: string,
  mode: 'correct' | 'complete'
): Promise<string> => {
  if (!genAI) return text;
  const prompt = mode === 'correct'
    ? `Correct the grammar and spelling of the following text in ${language}. Return ONLY the corrected text:\n\n${text}`
    : `Continue and complete the following text naturally in ${language}. Return ONLY the completed text:\n\n${text}`;
  try {
    return await callGemini(prompt) || text;
  } catch (e) {
    return text;
...
export const generateArticleContentFromTopic = async (
  topic: string,
  language: string,
  examples: KBArticle[]
): Promise<string> => {
  if (!genAI) return "";
  const prompt = `Write a detailed knowledge base article about "${topic}" in ${language}. 
Use markdown formatting with headers, bullet points and code blocks where appropriate.
Make it practical and easy to follow for IT users.`;
  try {
    return await callGemini(prompt);
  } catch (e) {
    return "";
...
Assistant:`;
  ...
  try {
    return await callGemini(prompt) || "Non ho potuto generare una risposta.";
  } catch (e) {
